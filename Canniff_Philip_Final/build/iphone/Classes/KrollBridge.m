...
NSString * Canniff_Philip_Final$ModuleRequireFormat = @"(function(exports){"
...
@implementation Canniff_Philip_FinalObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_canniff_philip_final);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Canniff_Philip_Final global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Canniff_Philip_Final" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_canniff_philip_final = [[Canniff_Philip_FinalObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_canniff_philip_final];
	
	NSString *_canniff_philip_finalNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _canniff_philip_finalNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_canniff_philip_final valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Canniff_Philip_Final$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.canniff_philip_final.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.canniff_philip_final.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
